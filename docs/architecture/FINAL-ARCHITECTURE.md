...
**อัปเดต (Phase 7 Architecture Decision, Locked 2026-08-16 — ดูรายละเอียดเต็มที่หัวข้อ 23 "NOTIFY_OWNER Delivery & Broadcast Test Send")**: เมื่อถึง Phase 7 จริง มีแค่ `SEND_NOTIFICATION` เท่านั้นที่ implement การส่งจริง — `NOTIFY_OWNER` ยังคง FAILED-seam behavior เดิมนี้ต่อไปอย่างไม่มีกำหนด (ไม่ใช่แค่ระหว่าง Phase 6) เพราะไม่มี Owner/Staff LINE identity model ที่นิยามไว้ในเอกสารนี้ ณ ตอนนี้
...
### Messaging API Channel Access Token — Phase 7 V1 Architecture Decision (Locked, 2026-08-16)

**ขอบเขต**: มตินี้ระบุ deviation เฉพาะขั้นตอนที่ 1 ของ "สิ่งที่ Platform Backend Automate ได้จริง" ด้านบน สำหรับ **Messaging API channel เท่านั้น** — ไม่กระทบ LINE Login channel (ยังคง issue token ผ่าน `client_credentials` ตามเดิม ยืนยันแล้วว่าใช้งานได้จริงกับบัญชีจริง) ไม่กระทบ field ใดของ `lineChannelConfigs` schema ในหัวข้อนี้ ไม่ override ข้อความอื่นใดที่มีอยู่เดิม:

- **สาเหตุ**: §35-mandated spike (ทดสอบกับบัญชี LINE Developer จริง ผ่าน Provider ที่เชื่อมกับ Shop_member OA @406plisw) ยืนยันว่า `POST /oauth2/v3/token` ด้วย `grant_type=client_credentials` **ใช้งานได้จริงกับ LINE Login channel** (ออก token สำเร็จ, ใช้สร้าง/ลบ LIFF app ผ่าน LIFF Server API ได้จริง) แต่ **ล้มเหลวซ้ำหลายครั้งกับ Messaging API channel ของบัญชีทดสอบนี้โดยเฉพาะ** (`invalid_request: Invalid 'client_credentials'.`) แม้จะลองทั้ง endpoint ทางการ (`/oauth2/v3/token`, legacy `/v2/oauth/accessToken`) และ Channel Secret ที่ reissue ใหม่หลายรอบแล้วก็ตาม — สรุปว่าเป็นข้อจำกัดเฉพาะ channel/account configuration นี้ ไม่ใช่ bug ของ implementation หรือปัญหาการเลือก endpoint
- **มติ V1**: ใช้ **Console-issued long-lived Channel Access Token** ของ Messaging API channel แทน — Owner คัดลอกค่านี้จาก Console (Messaging API tab → "Issue" long-lived token) มาวางในขั้นตอนเชื่อมต่อของ Dashboard **ครั้งเดียว** แทนที่จะให้ platform ออกเองผ่าน `client_credentials` ทุกครั้งที่ต้องใช้
- **Credential ที่ใช้**: `LINE_MESSAGING_CHANNEL_ACCESS_TOKEN` — ค่านี้เก็บผ่าน **Secret Manager reference เท่านั้น** (เช่นเดียวกับ `accessTokenRef` ที่มีอยู่แล้วใน `lineChannelConfigs.messagingChannel` schema ด้านบน — ไม่ต้องเพิ่ม field ใหม่ เพราะ schema เดิมออกแบบให้เป็น reference อยู่แล้วไม่ว่าค่าจะมาจากการ self-issue หรือจาก Console) **ห้าม hardcode ในโค้ด ห้ามเก็บใน client-accessible Firestore document ห้าม log/print/echo ค่านี้ที่ใดก็ตาม**
- **`LineAdapter` (Channel Adapter สำหรับ Messaging API)** อ่าน token ผ่าน Secret Manager reference นี้เพื่อเรียก Messaging API — ไม่มี logic ใดพยายาม self-issue token ของ Messaging API channel ผ่าน `client_credentials` ใน Phase 7 V1
- **Stateless/self-issued token สำหรับ Messaging API channel เป็น explicitly deferred item** — revisit ได้ในอนาคตถ้าหาสาเหตุที่แท้จริงเจอ (เช่น อาจต้องเปิด Console setting เพิ่มเติม หรือเปลี่ยน Provider/plan) ไม่ใช่ V1 scope; long-lived token ที่ Owner คัดลอกเองไม่มี expiry อัตโนมัติเหมือน stateless token (15 นาที) แต่ยังคง revoke ได้ผ่าน Console เช่นเดิมถ้าจำเป็น
- **ไม่กระทบ Onboarding Flow ของหัวข้อ 20** ในหลักการ — Owner ยังคงคัดลอกค่าจาก Console 4 ค่าตามเดิมสำหรับ Login channel + LIFF, บวกเพิ่มอีก 1 ค่า (long-lived Messaging Access Token) เฉพาะสำหรับ Messaging API channel ใน V1 นี้เท่านั้น (ดู note เพิ่มเติมที่ Step 4 ของหัวข้อ 20)

**เหตุผลที่ไม่ขัดกับ Architecture เดิม**: `lineChannelConfigs.messagingChannel.accessTokenRef` มีอยู่แล้วในหัวข้อนี้ตั้งแต่ต้น (ออกแบบเป็น reference ไม่ผูกกับวิธี issue) — มตินี้ระบุแค่ **แหล่งที่มา** ของค่าที่ reference นี้ชี้ไปสำหรับ Messaging API channel ใน V1 เท่านั้น ไม่เปลี่ยน schema ไม่เปลี่ยนหลักการ "ห้ามเก็บ secret ใน client-accessible Firestore document" ไม่เปลี่ยนหลักการ Secret Manager reference ใดๆ ที่มีอยู่แล้ว — สอดคล้องกับหลัก "ห้ามเดา business rule เกินความจำเป็น" และ pattern เดียวกับ Phase 5 Coupon Expiration/Phase 6 CHANGE_TIER/BIRTHDAY decisions ที่เลือกใช้ทางออกที่ตรงกับข้อจำกัดจริงที่พบระหว่าง implementation แทนการฝืนสมมติฐานเดิมที่พิสูจน์แล้วว่าใช้ไม่ได้กับบัญชีจริง
...
Step 4: คัดลอก 4 ค่าจาก Console มาวางใน Dashboard ของเรา:
        ...
        [Phase 7 V1 เพิ่มเติม — ดู "Messaging API Channel Access Token — Phase 7 V1 Architecture
        Decision" ด้านบน: Owner ต้องคัดลอกค่าที่ 5 เพิ่มด้วยคือ Messaging API channel's long-lived
        Channel Access Token (Console → Messaging API tab → Issue) เฉพาะ Messaging API channel
        เท่านั้น — LINE Login channel ยังคง "ไม่ต้อง copy token" ตามเดิม]

[ส่วนที่ Platform Backend ทำให้อัตโนมัติทั้งหมด — Owner กดปุ่มเดียวคือ "เชื่อมต่อ"]

Step 5: กด "เชื่อมต่อ" → Cloud Function:
        a) ออก Channel Access Token ด้วย client_credentials grant สำหรับ LINE Login channel;
           สำหรับ Messaging API channel ใน V1 ใช้ long-lived token ที่ Owner คัดลอกมาใน Step 4
           แทน (Phase 7 V1 Architecture Decision ด้านบน)
...
### NOTIFY_OWNER Delivery & Broadcast Test Send — Phase 7 Architecture Decision (Locked, 2026-08-16)

**ขอบเขต**: มตินี้ชี้แจงช่องทางส่งจริงของ `NOTIFY_OWNER` (action type ของหัวข้อ 16) และของขั้นตอน "Test Send" ใน Broadcast Flow ด้านบน — เอกสารก่อนหน้านี้ไม่เคยนิยาม LINE identity ของ Staff/Owner ไว้เลย (`merchantLineIdentity` เป็น field ระดับ Membership ของลูกค้าเท่านั้น ตามหัวข้อ 5/7 — `staffUsers` ไม่มี field ที่เกี่ยวกับ LINE ใดๆ) — ไม่กระทบ field/schema อื่นของ `automations`/`notificationSettings`/`staffUsers` ที่มีอยู่แล้ว ไม่ override ข้อความใดที่มีอยู่เดิม:

- **Phase 7 implement การส่งจริงสำหรับ `SEND_NOTIFICATION` เท่านั้น** (ลูกค้า — ใช้ `merchantLineIdentity` ที่มีอยู่แล้วในหัวข้อ 5/7) — เส้นทางนี้ได้ spec ครบอยู่แล้ว ไม่มีความกำกวม
- **`NOTIFY_OWNER` ยังคง deferred ต่อไปใน Phase 7** — ใช้พฤติกรรม Phase 6 FAILED-seam เดิมทุกประการ (execution record ถูกบันทึก `status:'FAILED'` พร้อม reason ที่อธิบายว่ายังไม่มีช่องทางส่งจริง) — ไม่ implement การส่งจริงสำหรับ action type นี้ใน Phase 7
- **ห้าม implement Owner/Staff LINE identity resolution หรือสร้าง schema ใหม่ใดๆ สำหรับ staff/owner identity ใน Phase 7** — `staffUsers` ต้องไม่มี field ใหม่ที่เกี่ยวกับ LINE (เช่น `lineUserId`) เพิ่มเข้ามาในเฟสนี้
- **Broadcast Flow's "Test Send"** ส่งไปยัง **test recipient ที่ Owner กำหนดค่าไว้เองอย่างชัดเจน** เก็บใน `notificationSettings/{merchantId}` (เช่น field `testRecipientLineUserId` — ค่า configuration ล้วนๆ ที่ Owner กรอกเอง ไม่ได้ resolve จาก identity ใดๆ ของระบบ) — **ไม่ใช้** `merchantLineIdentity`/`customerIdentities`/staff identity ใดๆ ในการ resolve ผู้รับ Test Send
- **Test recipient configuration นี้เป็น configuration เท่านั้น ห้ามสร้าง customer/staff identity semantics ใหม่** — ไม่ผ่าน `resolveOrCreatePlatformCustomer()`, ไม่สร้าง `customerIdentities`/`platformCustomers`/`memberships` record ใดๆ, ไม่ verify ผ่าน ID Token ตามหัวข้อ 21 (เพราะไม่ใช่ตัวตนของบุคคลที่ระบบต้อง track — เป็นแค่ปลายทางทดสอบข้อความที่ Owner พิมพ์เข้ามาเอง)
- Owner/Staff LINE identity resolution ที่แท้จริง (ถ้าต้องการให้ `NOTIFY_OWNER` ส่งจริงในอนาคต) เป็น **explicitly deferred item** รอ architecture/product decision cycle ใหม่ — ไม่ใช่ Phase 7 scope
- Phase 7 ยังต้องคง **tenant isolation, RBAC/Permission Matrix, server-side authorization, audit logging, idempotency, และ security requirements** ทั้งหมดตามที่ระบุไว้แล้วในหัวข้อ 3, 9, 10, 17, 18, 26, 27 — มตินี้ไม่เปลี่ยนแปลงหลักการเหล่านั้นแต่อย่างใด รวมถึง `testRecipientLineUserId` เองก็ต้องผ่าน `LINE_CONNECTION_MANAGE`-adjacent permission gate (Owner เท่านั้น ตามหัวข้อ 9) เช่นเดียวกับ configuration อื่นของ `notificationSettings`

**เหตุผลที่ไม่ขัดกับ Architecture เดิม**: `SEND_NOTIFICATION`/`NOTIFY_OWNER` ยังคงอยู่ใน `actions[].type` enum ของหัวข้อ 5/16 ครบตามเดิม (ไม่ลบ ไม่เปลี่ยน) — มตินี้ระบุแค่ขอบเขตการส่งมอบจริงของ Phase 7 ว่าครอบคลุมเฉพาะเส้นทางที่มี spec ครบ (`SEND_NOTIFICATION` ผ่าน `merchantLineIdentity`) สอดคล้องกับหลัก "ห้ามเดา business rule" ของ CLAUDE.md และหลักการเดียวกับที่ใช้ล็อก Phase 6 CHANGE_TIER/BIRTHDAY decisions — Test Send ที่ใช้ configuration แทน identity resolution เป็นการเติมช่องว่างของ field ที่ยังไม่นิยาม (`notificationSettings/{merchantId}` ในหัวข้อ 5 ไม่มี field list มาก่อน) ไม่ใช่การเปลี่ยนกติกาใด
